#!/usr/bin/env python3
"""
rentify/apartamentos.py — Valorius Market Intelligence
Extrae APARTAMENTOS de Rentify.hn y genera CSV revisable.
...
def extract_property(page, url: str) -> dict | None:
    ...
    # Rentify ya filtra status[]=a-la-venta y type[]=apartamentos-bienes-raices en la URL.
    # No aplicamos filtros internos de tipo ni estado.
    ...
    # para evitar que el scraper tome datos de otras propiedades
    for marcador in ["listing similar", "listings similar", "propiedades similar",
        ...
        if idx > 500:  # solo cortar si hay suficiente texto antes
            txt = txt[:idx]
            break
    ...
    # 2. Selectores HTML específicos de Rentify
    if not ubicacion_raw:
    ...
    # 3. Fallback: regex solo en los primeros 800 caracteres del texto
    if not ubicacion_raw:
        txt_head = txt[:800]
