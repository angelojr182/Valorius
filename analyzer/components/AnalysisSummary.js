...
var AnalysisSummary = (function() {
  ...
  function render(props) {
    ...
    var titulo = props.modoLimitado
      ? 'Referencia basada en pocas propiedades similares'
      : (iprInt.etiqueta || 'Resultado no disponible');
  ...
  function mount(elementId, props) {
    ...
    var clase = iprInt.clase && /^vrd-(bajo|rango|sobre|ref)$/.test(iprInt.clase)
      ? iprInt.clase
      : (categoria ? 'vrd-' + categoria : '');

    element.className = 'veredicto-principal' + (clase ? ' ' + clase : '');
...
if (typeof window !== 'undefined') window.AnalysisSummary = AnalysisSummary;
